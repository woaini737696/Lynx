...
mod webview2_link {
    ...
    pub fn output_libs(manifest_dir: PathBuf, out_dir: PathBuf) -> super::Result<PathBuf> {
        ...
        for target in WEBVIEW2_TARGETS {
            ...
            // GNU 工具链需要 lib{name}.a 命名格式，创建副本
            let target_env = env::var("CARGO_CFG_TARGET_ENV").unwrap_or_default();
            if target_env == "gnu" {
                use std::fs;
                let mut static_src = out_dir.clone();
                static_src.push(target);
                static_src.push("WebView2LoaderStatic.lib");

                let mut gnu_dest = out_dir.clone();
                gnu_dest.push(target);
                gnu_dest.push("libWebView2LoaderStatic.a");

                if static_src.exists() {
                    eprintln!("Creating GNU-format copy: {:?} -> {:?}", static_src, gnu_dest);
                    let _ = fs::copy(static_src.as_path(), gnu_dest.as_path());
                }
            }
        }
