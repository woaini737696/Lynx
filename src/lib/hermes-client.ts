 ...
 *
 * 安装策略（依次尝试镜像源，任一成功即返回）：
 *   `pip install hermes-agent -i <mirror>`
 * —— HermesAgent v0.14+ 已正式发布到 PyPI，纯 pip 安装本体几秒内完成
 * —— 多镜像源依次回退（清华 → 阿里 → 腾讯 → 官方），适配国内网络环境
 * —— 清除 PIP_INDEX_URL/PIP_EXTRA_INDEX_URL 环境变量，避免继承错误 index
 *
