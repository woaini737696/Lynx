...
function HermesConfigSection() {
  ...
  const checkLocalDashboard = async (): Promise<{ online: boolean; version?: string }> => {
    try {
      ...
      const res = await fetch("http://localhost:9119/api/status", { signal: ctrl.signal });
      clearTimeout(timer);
      if (res.ok) {
        const data = await res.json();
  ...
  // 前端直接检测本地 Dashboard 版本 + 对比服务器 latest.json
  const handleCheckUpdate = async () => {
    setCheckingUpdate(true);
    setUpdateInfo(null);
    try {
      // 1. 检测本地 Dashboard 版本（前端直接 fetch localhost）
      ...
      const hasUpdate = compareVersionsSimple(latestVersion, currentVersion) > 0;
      if (hasUpdate) {
        toast(`发现新版本 v${latestVersion}（当前 v${currentVersion}）`, "success");
        setUpdateInfo({
          currentVersion,
          latestVersion,
          hasUpdate: true,
        ...
        toast(`当前已是最新版本 v${currentVersion}`, "success");
        setUpdateInfo({
          currentVersion,
          latestVersion,
          hasUpdate: false,
    ...
    >
      ...
      <div className="mb-4 space-y-2">
        ...
        {updateInfo && updateInfo.hasUpdate && (
          <div className="ios-glass-sm mb-4 rounded-xl border border-northstar/30 p-4">
            <div className="mb-1.5 flex items-center gap-2 text-sm font-medium text-northstar">
            ...
            {updateInfo.releaseNotes && (
              <p className="mb-3 text-xs text-foreground/70">{updateInfo.releaseNotes}</p>
            )}
            <Button size="sm" onClick={handleUpdate} disabled={updating} className="gap-1.5">
              {updating ? (
                <><Loader2 className="h-3.5 w-3.5 animate-spin" /> 更新中（1-3分钟）...</>
              ) : (
                <><DownloadCloud className="h-3.5 w-3.5" /> 立即更新</>
              )}
            </Button>
          </div>
        )}
        {updateInfo && !updateInfo.hasUpdate && (
          <div className="ios-glass-sm mb-4 flex items-center gap-2 rounded-xl p-3 text-sm text-task">
