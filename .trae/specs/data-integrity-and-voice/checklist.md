...
## 任务 1：飞书机器人配置持久化
- [x] AISetting 表新增 `larkWebhookUrl` / `larkWebhookToken` 字段，`npx prisma db push` 同步成功
- [x] `/api/ai/settings` GET 返回两个新字段，PUT 接受并保存
- [x] `lark-bot/page.tsx` 从数据库拉取配置，保存写入数据库（不再用 localStorage）
- [x] 首次加载检测 localStorage 旧键，自动迁移到数据库后清除
- [x] `/api/lark-bot/test` 从数据库读 webhook 配置
- [x] `hermes-client.ts` 推送飞书逻辑从数据库读 webhook
- [x] 验证：保存配置→清理 localStorage→刷新页面，配置仍在

## 任务 2：e2e 脏数据清理 + 规范
- [x] `scripts/cleanup-e2e-data.ts` 脚本存在且可运行
- [ ] 运行清理脚本后，记忆图谱和各表无 `E2E*` 前缀脏数据（待连库运行）
- [x] `e2e/helpers/auth.ts` 新增 `cleanupTestData` 辅助函数
- [x] 所有 `e2e/*.spec.ts` 加 `afterEach` 清理钩子
- [ ] e2e 测试运行后数据库无 `E2E*` 残留（待跑 e2e 验证）
- [x] `DEVELOPMENT_SPEC.md` 新增「§1.5 数据持久化规范」
- [x] `DEVELOPMENT_SPEC.md` 新增「§1.6 自测数据清理规范」

## 任务 3：AI 助理全局悬浮入口
- [x] `AssistantDrawer.tsx` 组件存在，右侧抽屉桌面端 40% 宽度、移动端全屏
- [x] `AssistantFloatingButton.tsx` 右下角悬浮按钮 + `Alt+J` 快捷键监听
- [x] `layout.tsx` 全局挂载悬浮按钮 + 抽屉
- [x] AI 助理页面本身不显示悬浮按钮
- [x] 抽屉内 AI 助理功能完整（对话/技能/语音）
- [x] 验证：任意页面悬浮按钮可见、点击滑出、Alt+J 唤出/收起
