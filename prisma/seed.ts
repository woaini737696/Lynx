...
async function main() {
  ...
  const taskTemplates: Array<{ content: string; column: "northstar" | "campaign" | "task"; status: "active" | "done" | "dropped" }> = [
    ...
    { content: "灵感收敛定时器，每晚 23 点强制处理 inbox", column: "campaign", status: "done" },
  ...
  const conversationData = [
    ...
    },
    {
      source: "claude",
      title: "灵感收敛交互流程",
      rawContent: "【用户】: 灵感收敛怎么设计交互？\n【AI】: 每晚 23 点弹窗强制处理 inbox，每个灵感必须选择：看板/延后/墓地。用机制替代意志力。",
      conclusions: ["灵感收敛固定 23 点触发", "每个灵感必须三选一：看板/延后/墓地"],
  ...
  const cognitionData = [
    ...
    { type: "method", content: "灵感收敛用机制替代意志力，固定时间强制处理inbox", source: "conversation", tags: ["方法论", "习惯"] },
  ...
  const memoryData = [
    ...
    { type: "conversation", content: "灵感收敛交互流程", strength: 0.76 },
    ...
    { type: "cognition", content: "灵感收敛机制替代意志力", strength: 0.79 },
