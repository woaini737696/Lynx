...
- **禁止断档**：DEV_LOG.md 必须与实际迭代一一对应，不允许出现"迭代到 N 就停止更新"的情况；每次会话开始前先检查最新迭代号，本次迭代号必须 = 最新迭代号 + 1
- **日志查看页必须分页**：`/dev-log` 页面必须支持分页（默认 5 条/页，可配置）+ 时间筛选（按日期下拉）+ 关键词搜索（匹配迭代号/标题/正文）
- **日志 API 结构化**：`/api/dev-log` 必须返回结构化迭代数组（`entries: [{number, date, title, rawContent}]`），前端基于数组做分页/筛选，禁止一次性渲染全部
- **验证**：开发结束前检查 `DEV_LOG.md` 顶部迭代号是否等于本次迭代；检查 `/dev-log` 页面分页/筛选功能正常
...
- **列表页（强制分页）**：所有列表类页面必须实现分页，无例外
  - 默认 10 项/页（日志类 5 项/页），必须提供 pageSize 切换（5/10/20/50）
  - 必须包含搜索框（`SearchInput` 组件）+ 筛选下拉（`FilterSelect` 组件，按分类/状态/日期等）
  - 必须使用 `useClientPagination` Hook（客户端分页）或服务端分页
  - 适用范围：用户管理、角色管理、职业空间、开发日志、词元统计、认知库、技能列表、灵感、任务、记忆、巡检日志、对话记录、所有 admin 列表等
  - **禁止**：一次性渲染全部数据，无论数据量大小
- **左侧导航栏**：内容区滚动时保持固定
- **3D 记忆图谱**：支持滚轮缩放 + 节点点击聚焦
- **移动端**：任务数据使用 database-only 模式（`db_only=true`），不依赖 lark-cli
- **弹窗层级**：Modal 必须使用 `z-[200]` 层级 + `bg-black/50 backdrop-blur-sm` 遮罩，确保展示在最顶层可见，禁止被其他元素遮挡
...
### 4.3.1 账号保护规范（强制，禁止违反）
- **用户主账号**：`lynn` / `ee9527ff`（role=admin，displayName=Lynn），是项目所有者本人凭证
- **禁止擅自修改**：未经用户明确书面同意，**任何 Agent/脚本/工具不得修改 lynn 账号的密码、role、displayName、active 状态**，也不得删除该账号
...
- **本地 vs 云端**：本地后端 (localhost:5176) 与生产 (ai.lynxdo.com) 共用同一套账号体系；lynn 账号在两端均有效，密码由用户统一维护
...
- **远程 IPC 访问**：Web UI 从 `localhost:5176`（开发）或 `ai.lynxdo.com`（生产）加载时，必须通过 `capabilities/default.json` 的 `remote.urls` 授权才能调用 Tauri 命令；禁止使用已废弃的 `dangerousRemoteDomainIpcAccess`（Tauri 1.x 字段，v2 不再生效）
...
- **后端 endpoint 切换**：开发期 `frontendDist` 指向 `localhost:5176`，生产部署后切换为 `https://ai.lynxdo.com`，实现「先本地跑通再部署云端」的独立安装产品形态
...
- `ai.lynxdo.com` → Web 应用 + API（Nginx 反代到 Next.js :5176）
- `ai.lynxdo.com/download/` → 桌面端安装包下载（Nginx 静态目录）
...
### 14.6 部署验证清单
每次部署后必须逐项验证：
1. `curl https://ai.lynxdo.com/api/health` → `{"ok":true}`
2. 浏览器访问 `https://www.lynxdo.com` → 官网首页
3. 浏览器访问 `https://ai.lynxdo.com` → 登录页
